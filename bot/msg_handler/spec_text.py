...
class special_text_handler(object):
    ...
    def handle_text(self, event):
        ...
        for spec_kw, spec in self._special_keyword.iteritems():
            search_result = re.search(spec_kw, msg_text)

            if search_result:
                spec_func, spec_param = spec
                rep_text = spec_func(*(spec_param + (uid, cid, search_result)))
    ...
    def _handle_text_spec_luck(self, uid, cid, search_result):
        ...
        if search_result.lastindex >= 1:
            ...
            if count > MAX_ONCE_COUNT:
                ...
                result.append(u'總分 {:.5f} ({:+.5f}) | 標準差 {:.5f}'.format(score_list_stats.sum, score_list_stats.sum - 2.501499 * score_list_stats.count, score_list_stats.std))
            ...
            result = u'運勢分數(0~10): {:.5f}\n高於此分數的機率: {:.5%}'.format(score, score_package.get_opportunity_greater())

        return u'{}\n{}'.format(result, self.__get_max_scores_str())

    def __get_max_scores_str(self):
        now = datetime.now()
        ...
        return u'{}日內最高分: {:.5f} ({:.3%})\n本日最高分(AM 8起算): {:.5f} ({:.3%})\n\n其他運勢使用方式請參見說明書。\n運勢專區/小水母洗版區: https://line.me/R/ti/g/uI3tAfrqvE'.format(
            ...
            float(str(m_score)), 
            game.sc_gen_data.calculate_opportunity_greater(m_score), 
            float(str(m_score_1d)), 
            game.sc_gen_data.calculate_opportunity_greater(m_score_1d))

    def _handle_text_spec_luck_rec(self, uid, cid, search_result):
    ...
    def _handle_text_spec_luck_self(self, uid, cid, search_result):
        if uid is not None:
    ...
    def _handle_text_spec_time(self, uid, cid, search_result):
    ...
    def _handle_text_spec_rand_wife(self, uid, cid, search_result):
